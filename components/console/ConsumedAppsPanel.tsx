...
export default function ConsumedAppsPanel({
  ...
  const { user } = useAuth();
  const usage = useAccountUsage(user?.id?.trim(), {
    window: "mtd",
    includePrior: false,
  });
  ...
  if (usage.status === "loading" || usage.status === "idle") {
    return (
      <PanelShell>
        <div className="animate-pulse border-b border-hairline px-5 py-4" aria-hidden="true">
    ...
    return (
      <PanelShell>
        ...
        <div className="px-5 py-8 text-center">
          ...
          <p className="mt-1 font-mono text-[11px] text-fg-faint">{usage.message}</p>
          <button
            type="button"
  ...
  const { apps, totalSpendDisplay, totalCalls7d } = buildOrgConsumptionFromUsage(
    usage.data,
    organization,
  );
  ...
  return (
    <PanelShell>
      <PanelHeader totalSpendDisplay={totalSpendDisplay} totalCalls7d={totalCalls7d} />
      <ColumnHeaders />
