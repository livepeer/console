...
import { PIPELINE_UTILIZATION, LIVE_JOBS } from "@/lib/console/mock-data";
import type {
  NetworkStat,
  PipelineUtilization,
  LiveJobStatus,
...
function StatusBadge({
  ...
  status: PipelineUtilization["status"] | LiveJobStatus;
}) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[11px] ${bg} ${text}`}
    >
      <span
        className={`h-1.5 w-1.5 rounded-full ${status === "online" || status === "active" ? "bg-green-bright" : status === "degraded" ? "bg-yellow-400" : "bg-fg-faint"}`}
      />
      {label}
    </span>
  );
...
function LiveJobFeed() {
  ...
  const activeJobCount = LIVE_JOBS.filter(
            ...
            return (
              <div key={job.id}>
                ...
                <div className="hidden md:flex min-w-[600px] items-center gap-4 px-4 py-2.5 transition-colors hover:bg-zebra">
                  <span className="flex-1 text-sm text-fg-strong">
                    {job.pipeline}
                  </span>
                  <span className="w-44 truncate text-xs text-fg-faint">
                    {job.model}
                  </span>
                  <span className="w-20 text-right text-xs text-fg-faint">
                    {fps}
                  </span>
                  <span className="w-14 text-right text-[11px] text-fg-label">
                    {job.age}
                  </span>
                  <span className="w-20">
...
export default function UtilizationTab() {
  ...
  const kpi: NetworkStat[] = useMemo(() => {
    ...
    const totalWarm = PIPELINE_UTILIZATION.reduce(
      (s, p) => s + p.warmOrchestrators,
      0
    );
  ...
  const pipelineTotalPages = Math.ceil(
    filteredPipelines.length / PIPELINE_PAGE_SIZE
  );
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5 text-[12px] text-fg-muted">
                ...
                warm ·{" "}
                {Math.round(
                  PIPELINE_UTILIZATION.filter(
                    (p) => p.status !== "cold"
                  ).reduce((s, p) => s + p.utilizationPct, 0) /
                    PIPELINE_UTILIZATION.filter((p) => p.status !== "cold")
                      .length
                )}
                % avg utilization
              </p>
                ...
                <div key={p.id}>
                  ...
                  <div className="flex flex-col gap-2 px-4 py-3 transition-colors hover:bg-zebra md:hidden">
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[11px] text-fg-faint">
                      ...
                      <span>
                        <span className="text-fg-disabled">Price </span>$
                        {p.price}
                        {p.priceUnit}
                      </span>
                    </div>
          ...
          <div className="flex items-center justify-between border-t border-hairline px-4 py-2.5">
            ...
            <div className="flex gap-1.5">
              ...
              <button
                ...
                onClick={() =>
                  setPipelinePage(
                    Math.min(pipelineTotalPages - 1, pipelinePage + 1)
                  )
                }
