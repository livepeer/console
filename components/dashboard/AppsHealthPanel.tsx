...
export default function AppsHealthPanel() {
  ...
  const sorted = [...apps].sort(
    (a, b) =>
      STATUS_META[a.deployment.status].rank -
        STATUS_META[b.deployment.status].rank ||
      b.deployment.calls7d - a.deployment.calls7d,
  );
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-hairline bg-dark-lighter shadow-card">
      ...
      {sorted.map((a: Pipeline, i) => {
        const s = STATUS_META[a.deployment.status];
        return (
          <Link
            key={a.id}
          ...
          >
            ...
            <span className="min-w-0">
              ...
              <span className="mt-0.5 block truncate font-mono text-[11px] text-fg-faint">
                {a.deployment.pipelineId}
              </span>
            </span>
            <span className="hidden items-center gap-1.5 text-[12.5px] text-fg-strong sm:inline-flex">
              <StatusDot tone={s.dot} static={a.deployment.status !== "deployed"} />
              {s.label}
            </span>
            <span
              className={`hidden sm:block ${a.deployment.calls7d > 0 ? s.spark : "text-fg-disabled"}`}
            >
              <Sparkline data={seededSpark(a.id, a.deployment.status)} className="" />
            </span>
            <span className="text-right font-mono text-[12.5px] tabular-nums text-fg-strong">
              {formatCompact(a.deployment.calls7d)}
            </span>
            <span className="text-right font-mono text-[12.5px] tabular-nums text-fg-strong">
              {a.deployment.p50LatencyMs > 0 ? `${a.deployment.p50LatencyMs}ms` : "—"}
            </span>
          </Link>
        );
      })}
