...
import StackedAreaChart, { MiniSpark } from "@/components/dashboard/StackedAreaChart";
...
function UsageLoadError({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center px-5 py-24 text-center">
      <p className="text-sm text-fg-muted">Could not load usage from PymtHouse.</p>
      ...
      <div className="flex flex-wrap items-baseline gap-3">
        {showUsdAllowance ? (
          <span className="font-mono text-[15px] tabular-nums leading-none text-fg-muted">
            ...
            <span className="text-fg-faint"> / ${microsToUsdDisplay(granted)} remaining</span>
          </span>
        ) : requestLimit ? (
          <span className="font-mono text-[15px] tabular-nums leading-none text-fg-muted">
            <b className="mr-0.5 text-[24px] font-medium tracking-[-0.01em] text-fg">
        ...
        <div className="relative h-1.5 rounded-[3px] bg-dark-card">
          <div
            className={`h-full rounded-[3px] ${
              hasAccess ? "bg-gradient-to-r from-green to-green-bright" : "bg-warm"
            }`}
            style={{ width: `${pct}%` }}
          />
          {requestLimit && (
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-4 text-[12px] text-fg-muted">
        {requestLimit && willExceed ? (
          <span className="font-mono">
            Forecast <b className="font-medium text-fg">{fmt(forecast)}</b> jobs by {resetsAt} ·
            over limit in <b className="font-medium text-warm">~{daysToLimit}d</b>
          </span>
        ) : (
          <span className="font-mono">
            <b className="font-medium text-fg">{fmt(requestCount)}</b> signed requests this period
            {showUsdAllowance && (
              <>
                {" "}
                · <b className="font-medium text-fg">${microsToUsdDisplay(usedUsd)}</b> consumed
              </>
            )}
          </span>
        )}
  ...
  if (usageState.status === "loading" || usageState.status === "idle") {
    return (
      <div className="mx-auto w-full max-w-[1200px] px-7 pb-20 pt-7">
        <DashboardPageSkeleton maxWidth="6xl" withTabs={false} kpiCount={0} withChart />
      </div>
    );
  }
...
function BreakdownTable({
  ...
  return (
    <div className="overflow-hidden rounded-md border border-hairline bg-dark-lighter shadow-card">
      ...
      {rows.length === 0 ? (
        <p className="px-4 py-8 text-center text-sm text-fg-faint">No matching capabilities.</p>
      ) : (
        rows.map((c) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-end gap-2.5">
                <span className="font-mono tabular-nums text-fg-strong">{fmt(c.requestCount)}</span>
                <span className="opacity-85">
                  <MiniSpark data={c.data} color={c.color} height={18} width={70} />
                </span>
              </div>
              <div
              ...
              <div className="justify-self-end font-mono tabular-nums text-fg">
                {fmtSpend(c.spendUsd)}
                {unitCost > 0 && (
                  <span className="text-fg-disabled"> · ${unitCost.toFixed(4)}/req</span>
                )}
              </div>
            </div>
          );
      ...
      <div className={`${cols} bg-dark py-2.5 text-[13px] font-medium text-fg-strong`}>
        <div>
          Total
          <span className="ml-1.5 text-[11px] text-fg-faint">· this period</span>
        </div>
        <div className="justify-self-end font-mono tabular-nums text-fg">{fmt(grandReq)}</div>
        <div />
        <div />
        <div />
        <div className="justify-self-end font-mono tabular-nums text-fg">{fmtSpend(grandSpend)}</div>
      </div>
    </div>
  );
}
