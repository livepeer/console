...
import { OWNED_APPS } from "@/lib/dashboard/mock-data";
...
function appLines(app: Pipeline): { level: Level; msg: string }[] {
  if (app.deployment.status === "building") {
    return [
      { level: "info", msg: `building image ${app.deployment.image.split("/").pop()}` },
  ...
  if (app.deployment.status === "error") {
    return [
  ...
  return [
    { level: "info", msg: `POST /predict · 200 · ${app.deployment.p50LatencyMs || 60}ms` },
...
function buildLogs(apps: Pipeline[]): LogLine[] {
  const active = apps.filter((a) => a.deployment.status !== "stopped");
...
export default function LogsView() {
  ...
  const apps = useMemo(() => {
    ...
    return OWNED_APPS.filter((p) => {
      if (seen.has(p.deployment.pipelineId)) return false;
      seen.add(p.deployment.pipelineId);
      return true;
    });
  }, []);
  const activeApps = apps.filter((a) => a.deployment.status !== "stopped");
