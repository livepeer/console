...
export async function ensurePreviewRunFixtures(
  ...
  if (!existing.has(ids[2]!)) {
    const run = await createRun(owner, {
      ...
      submittedArguments: {
        capability: "meta-llama/llama-3.1-8b-instruct",
        inputs: {
          prompt: "Write a one-line gallery caption for an emerald portrait.",
          max_tokens: 80,
          temperature: 0.6,
        },
      },
  ...
  return { runIds: ids, createdRunIds: created, createdCount: created.length };
}
