...
function parseRunnerPaymentChallenge(
  error: RunnerHttpError
): RunnerPaymentChallenge {
  let data: unknown;
  try {
    data = JSON.parse(error.body);
  } catch {
    throw new RunnerGatewayError(
      "Live runner payment challenge response was not valid JSON",
      {
  ...
  if (!data || typeof data !== "object" || Array.isArray(data)) {
    throw new RunnerGatewayError(
      "Live runner payment challenge response must be a JSON object",
      {
        code: "payment_challenge_invalid",
        status: 502,
      }
  ...
  const orchestratorUrl =
    typeof record.orchestrator === "string" ? record.orchestrator : "";
  const manifestId =
    typeof record.manifest_id === "string" ? record.manifest_id : "";
  if (!paymentParams || !orchestratorUrl || !manifestId) {
    throw new RunnerGatewayError(
      "Live runner payment challenge missing required fields",
      {
        code: "payment_challenge_invalid",
        status: 502,
      }
...
async function getRunnerPayment(
  ...
  const url = signerEndpointUrl(
    signer.signerUrl,
    DIRECT_SIGNER_PATHS.generateLivePayment
  );
  ...
  if (!payment || !segCreds) {
    throw new RunnerGatewayError(
      "Payment response missing payment or segCreds",
      {
        code: "signer_error",
        status: 502,
      }
...
export async function callRunner(input: {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    if (challenge && signer) {
      const payment = await getRunnerPayment(
        challenge,
        signer,
        paymentAttribution
      );
    ...
    try {
      ...
      if (response.status === 402) {
        if (!signer) {
          throw new RunnerGatewayError(
            "Live runner paid call requires signer",
            {
  ...
  throw new RunnerGatewayError(
    "Live runner call exhausted payment challenge retries",
    {
...
export async function callRunnerStream(input: {
  ...
  throw new RunnerGatewayError(
    "Live runner stream call exhausted payment challenge retries",
    {
      code: "payment_challenge_exhausted",
      status: 502,
    }
...
export async function reserveSession(input: {
  ...
  const { discoverRunnerCandidates } =
    await import("@/lib/runner-gateway/discovery");
    ...
    try {
      ...
      const sessionId =
        typeof result.data.session_id === "string"
          ...
          : "";
      const appUrl =
        typeof result.data.app_url === "string"
          ? result.data.app_url.trim()
          : "";
      if (!sessionId) {
        throw new RunnerGatewayError(
          "runner session response missing session_id",
          {
            code: "runner_error",
            status: 502,
          }
      ...
      if (!appUrl) {
        throw new RunnerGatewayError(
          "runner session response missing app_url",
          {
            code: "runner_error",
            status: 502,
          }
