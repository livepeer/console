...
import { errorMessageFromBody, isAccountUsagePayload } from "./account-usage-payload";

function validPayload(): Record<string, unknown> {
  return {
    clientId: "app_dashboard",
    period: { start: "2026-07-08T00:00:00.000Z", end: "2026-08-07T00:00:00.000Z" },
    periodDayKeys: ["2026-08-06", "2026-08-07"],
    priorPeriod: { start: "2026-06-08T00:00:00.000Z", end: "2026-07-08T00:00:00.000Z" },
    balance: null,
    current: {
      requestCount: 3,
...
test("rejects scopes whose BigInt/array reads would throw during render", () => {
  ...
  noMicros.current = { ...(validPayload().current as object), endUserBillableUsdMicros: 2400 };
  assert.equal(isAccountUsagePayload(noMicros), false);

  const noRows = validPayload();
  noRows.current = { ...(validPayload().current as object), pipelineModels: null };
  ...
  rowWithoutSeries.current = {
    ...
    pipelineModels: [{ pipeline: "text-to-image", modelId: "sd-3", requestCount: 3 }],
  };
...
test("errorMessageFromBody only trusts a string error field", () => {
  assert.equal(errorMessageFromBody({ error: "upstream down" }), "upstream down");
